...
import Persistencia.ControladorDatosRelaciones;
import Persistencia.ControladorDatosTareas;
...
public class ControladorTareas {
    ...
    public void cargarDatosTareas() throws IOException
    {
        ControladorDatosTareas CntrlDS = new ControladorDatosTareas();
        CntrlDS.abrirArchivoTareas();
        
        String[][] ret = CntrlDS.leerTareas();
        ArrayList<Tarea> tareas = new ArrayList<Tarea>();
        for (int i = 0; i < ret.length; i++) {
            Tarea t = new Tarea(
                    Integer.parseInt(ret[i][0]), 
                    ret[i][1], 
                    Double.parseDouble(ret[i][2]), 
                    Double.parseDouble(ret[i][3]), 
                    Double.parseDouble(ret[i][4]), 
                    Double.parseDouble(ret[i][5]), 
                    ret[i][6]);
            tareas.add(t);
        }
        
        this.CjTareas = new HashMap<Integer,Tarea>();
        this.tareaRelacionCoste = new HashMap<Integer,ArrayList<Integer>>();
        this.tareaRelacionTiempo = new HashMap<Integer,ArrayList<Integer>>();
        this.contIdTarea = 0;
        for (int i = 0; i < tareas.size(); i++) {
            if(tareas.get(i).id() > this.contIdTarea) this.contIdTarea = tareas.get(i).id();
            CjTareas.put(tareas.get(i).id(), tareas.get(i));
            this.anadirTarea(tareas.get(i));
        }
        CntrlDS.abrirArchivoTareas();
    }
    
    public void guardarDatosTareas() throws IOException
    {
        ControladorDatosTareas CntrlDS = new ControladorDatosTareas();
        String[][] ret = new String[CjTareas.size()][7];
        
        ArrayList<Integer> idTareas = new ArrayList<Integer>(CjTareas.keySet());
        
        for(int i = 0; i < idTareas.size(); ++i) {
            Tarea t = CjTareas.get(idTareas.get(i));
            ret[i][0] = String.valueOf(t.id());
            ret[i][1] = t.nombre();
            ret[i][2] = String.valueOf(t.tiempoProduccion());
            ret[i][3] = String.valueOf(t.tiempoPreparacion());
            ret[i][4] = String.valueOf(t.costeProduccion());
            ret[i][5] = String.valueOf(t.costePreparacion());
            ret[i][6] = String.valueOf(t.descripcion());
        }
        
        CntrlDS.abrirArchivoTareas();
        CntrlDS.modificarArchivo(ret);
        CntrlDS.cerrarArchivoTareas();
    }
    
    public void cargarDatosRelaciones() throws IOException
    {
        this.CjRelaciones = new HashMap<Integer,Relacion>();
        this.contIdRel = 0;
        ControladorDatosRelaciones CntrlDS = new ControladorDatosRelaciones();
        CntrlDS.abrirArchivoRelaciones();
        
        String[][] ret = CntrlDS.leerRelaciones();
        for (int i = 0; i < ret.length; i++) {
            Boolean tipo;
            if(Integer.parseInt(ret[i][3]) == 1) {
                tipo = Boolean.TRUE;
            }
            else {
                tipo = Boolean.FALSE;
