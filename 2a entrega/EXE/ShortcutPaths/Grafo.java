...
//    }
    ...
    public Grafo(Grafo original) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            this.listasAdyacencias.put(i, new ArrayList<>((ArrayList<Vertice<T>>)original.listasAdyacencias.get(i)));

            for (int j = 0; j < n; j++) {
                Arista a = null;
                if(i!=j && original.adyacentes(i, j)) a = new Arista(original.arista(i, j));
                if(i==0 && j==0) this.matrizAdyacencias.get(0).set(0, a);
                else this.matrizAdyacencias.get(i).add(a);
            }
        }
    }
...
//    }
    ...
    public int gradoVertice(int id) throws IllegalArgumentException {
        if(!this.existeIdVertice(id)) throw new IllegalArgumentException("Error al consultar grado vértice: identificador no válido.");
...
//    }
    ...
    public ArrayList<Arista> aristasIncidentes(int id) throws IllegalArgumentException {
        if(!this.existeIdVertice(id)) throw new IllegalArgumentException("Error al consultar aristas incidentes: id de vértice no existe.");
        ArrayList<Arista> aristas = new ArrayList<>();
        for(int j=0;j<this.nVertices;j++) if(id!=j && this.adyacentes(id, j)) aristas.add(this.arista(id, j));
        return aristas;
    }
...
//    }
    ...
    private ArrayList<Vertice> verticesAdyacentes(int id) throws IllegalArgumentException {
        if(!this.existeIdVertice(id)) throw new IllegalArgumentException("Error al consultar vértices adyacentes: id de vértice no existe.");
        ArrayList<Vertice> vAd = new ArrayList<>();
        for(int j=0;j<this.nVertices;j++) if(id!=j && this.adyacentes(id, j)) vAd.add(this.vertice(j));
        return vAd;
    }
    ...
    public ArrayList<Vertice> verticesAdyacentes(Vertice v) throws IllegalArgumentException {
        if(!this.existeVertice(v)) throw new IllegalArgumentException("Error al consultar vértices adyacentes: vértice no existe.");
        return this.verticesAdyacentes(this.idVertice(v));
    }

     /**
     * Devuelve uno de los caminos posibles con menor longitud entre dos vértices. 
     * El camino será un array con los indentificadores de los vértices que forman el camino. Si no existe camino, devuelve un array vacío.
     * @param idOrigen Identificador de un vértice.
     * @param idDestino Identificador de un vértice.
     * @return Uno de los caminos posibles con menor longitud entre dos vértices.
     */ 
    public ArrayList<Integer> camino(int idOrigen, int idDestino) throws IllegalArgumentException {
        ...
        if(idOrigen==idDestino) throw new IllegalArgumentException("Error en camino(int,int): deben ser identificadores diferentes.");
        ...
        for(int i=0;i<visitado.length;i++) visitado[i]=false;
    ...
    public boolean conectaTodosVertices(ArrayList<Arista> aristas) throws IllegalArgumentException {
        ...
        for(int i=0;i<aristas.size() && visitados<this.nVertices;i++) {
            ...
            if(a == null) throw new IllegalArgumentException("Error al comprobar arista: referencia nula a la arista.");
            int idOrigen = a.origen();
            int idDestino = a.destino();
            if(idOrigen==idDestino) throw new IllegalArgumentException("Error al comprobar arista: la arista tiene el mismo origen y destino.");
            if(!(this.existeIdVertice(idOrigen) && this.existeIdVertice(idDestino))) 
    ...
    public boolean completo(){
        ...
        for(int i=0;i<this.nVertices && esCompleto;i++)
            for(int j=i+1;j<this.nVertices;j++)
                if(this.matrizAdyacencias.get(i).get(j) == null) esCompleto = false;
        return esCompleto;
    }
    ...
    public boolean euleriano(){
        for(int g : this.grados) if(g%2 == 1) return false;
        return this.conexo();
    }
    ...
    public boolean adyacentes(int idOrigen, int idDestino) throws IllegalArgumentException {
        ...
        if(idOrigen==idDestino) throw new IllegalArgumentException("Error en adyacentes(int,int): deben ser identificadores diferentes.");
...
//    }
     ...
     * @param idDestino Indentificador del vértice destino incidente en la arista.
    ...
    public Arista arista(int idOrigen, int idDestino) throws IllegalArgumentException {
        ...
        if(idOrigen==idDestino) throw new IllegalArgumentException("Error al consultar una arista: deben ser identificadores diferentes.");
...
//    }
     ...
     /**
     * Pone una arista en el grafo que incidirá en los vértices idOrigen y idDestino. Si ya hay una, la sustituye.
     * @param idOrigen Identificador de un vértice.
     * @param idDestino Identificador de un vértice.
    ...
    private void ponerArista(int idOrigen, int idDestino, Arista arista) throws IllegalArgumentException {
        ...
        if(idOrigen==idDestino) throw new IllegalArgumentException("Error al poner una arista: deben ser identificadores diferentes.");
        if(!this.adyacentes(idOrigen, idDestino)) {
        ...
        if(!listAdyOri.contains(destino)) listAdyOri.add(destino);
        if(!listAdyDest.contains(origen)) listAdyDest.add(origen);
        
        this.nAristas++;
    }
    
     /**
     * Pone una arista en el grafo que incidirá en los vértices arista.origen() y arista.destino(). Si ya hay una, la sustituye. 
     * @param arista Arista que se va a añadir al grafo.
     */
    public void ponerArista(Arista arista) throws IllegalArgumentException {
        if(!(this.existeIdVertice(arista.origen()) && this.existeIdVertice(arista.destino()))) 
            throw new IllegalArgumentException("Error al poner una arista: algún vértice no existe.");
        if(arista.origen()==arista.destino()) throw new IllegalArgumentException("Error al poner una arista: deben ser identificadores diferentes.");
    ...
    public void eliminarArista(int idOrigen, int idDestino) throws IllegalArgumentException {
        ...
        if(idOrigen==idDestino) throw new IllegalArgumentException("Error al eliminar una arista: deben ser identificadores diferentes.");
        if(!this.adyacentes(idOrigen, idDestino)) 
